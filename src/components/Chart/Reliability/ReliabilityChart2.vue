...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async onChangePMIntervalOption() {
                let points
                let cumulativePoints
                if (this.selectedIntervalIndex !== this.defaultSelectedIntervalIndex) {
                    await CalculationAPI.reliabilityOfSubsequentPM(this.pmIntervalValues, this.failureData.distribution, this.failureData.parameters)
                        .then(({reliability, cumulativeReliability, failures}) => {
                            points = reliability
